...
// Question 3: Fetch only 3 posts for a given userId.
                async function fetch3PostByUserId(usersIds) {
                    let allRecord=await getAllUsersData()
                    let thirdEmptystr ="";
                    let secondEmptystr ='';
                    let firstEmptystr ='';
                    for(let i=0; i<=allRecord.length-1; i++){
                        if(allRecord[i].userId==usersIds){
                           thirdEmptystr =secondEmptystr;
                            secondEmptystr =firstEmptystr;
                            firstEmptystr =allRecord[i];
                            // empty.push(allRecord[i]) 
                              }
                        }      
                }
                // let getOnly3Records = 
                await fetch3PostByUserId(1)
